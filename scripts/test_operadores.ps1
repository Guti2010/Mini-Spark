...
function Run-JobAndWait {
    ...
    if ($estadoFinal -ne $ExpectedEstado) {
        Write-Host "TEST FALLÓ: se esperaba $ExpectedEstado pero fue $estadoFinal" -ForegroundColor Red
        exit 1
    } else {
...
$jobMapBad = Run-JobAndWait -JobName "test-map-bad-input" -ExpectedEstado "FAILED"
Show-Results -JobId $jobMapBad

Write-Host ""
Write-Host "#### TEST 8: REDUCE - VALOR NO NUMERICO (DEBE FALLAR) ####" -ForegroundColor Magenta
$jobReduceBad = Run-JobAndWait -JobName "test-reduce-bad-value" -ExpectedEstado "FAILED"
Show-Results -JobId $jobReduceBad

Write-Host ""
Write-Host "#### TEST 9: JOIN - CLAVE FALTANTE (DEBE FALLAR) ####" -ForegroundColor Magenta
$jobJoinBad = Run-JobAndWait -JobName "test-join-missing-key" -ExpectedEstado "FAILED"
